...
# CONFIG
# 4/II[X]; 4/II,V1[X]; 2/II[X]; 2/II,V1,V5[ ]; 1/None[X]
COLUMNS="2"
FULL_MODE="II,V1,V5"
...
BASE_DIR="/mnt/processed1/signals/CINC_CHALLENGE_2024/GENERATED_WFDB_FILES/CODE15/"
FULL_MODE_SAVE=$(echo "$FULL_MODE" | sed 's/,/_/g')
# SAVE_DIR="/mnt/processed1/signals/CINC_CHALLENGE_2024/GENERATED_IMG/CODE15/${COLUMNS}_${FULL_MODE_SAVE}"
SAVE_DIR="/mnt/processed2/signals/cinc2021_image/documents/CINC_CHALLENGE_2024/GENERATED_IMG/CODE15/${COLUMNS}_${FULL_MODE_SAVE}"

declare -a ALL_PATHS
index=0
for SUBFOLDER in "$BASE_DIR"/*/; do
    for SUBSUBFOLDER in "$SUBFOLDER"*/; do
        ALL_PATHS[index]="${SUBSUBFOLDER%/}"  # Remove trailing slash for consistency
        index=$((index+1))
    done
done
